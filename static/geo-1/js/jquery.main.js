...
function initLineChart() {
	...
	jQuery('.weekly-ranking').each(function() {
		var holder = jQuery(this);
		var chartHolder = holder.find('.graph-box');
		var slider = holder.find('.country-slider');
		var btnPrev = holder.find('.slick-prev');
		var btnNext = holder.find('.slick-next');
		var countryTitle = holder.find('.country-title');
		...
		function afterLoad() {
			...
			lineData = allData[holder.data('def-country')];
			countryTitle.text(holder.data('def-country'));
		...
		function formatData() {
			loadData.forEach(function(d) {
				...
				if (!allData[propName]) {
					...
					var slide = jQuery('<div data-name="' + propName + '">' + propName + '</div>');

					slide.appendTo(slider);
					countryItems = countryItems.add(slide);
				}
		...
		function attachEvents() {
			countryItems.on('click', function() {
				...
				var countryName = item.data('name');

				countryTitle.text(countryName);
				lineData = allData[countryName];
				chartHolder.empty();
				drawChart();
			});
		...
		function drawChart() {
			...
			yScale = d3.scaleLinear()
				.range([0, height])
				.domain([0, round(d3.max(lineData, function(d) {
					return +d.cases;
				}), 20, 20)]);
			...
			labels = wrapper.selectAll('.text')
				...
				.attr('dy', function(d, i) {
					...
					if (lineData[i + 1] && +d.cases > +lineData[i + 1].cases) {
						if (+lineData[i + 1].cases - +d.cases > 5) {
							value = '-4';
						} else {
							value = '12';
						}
